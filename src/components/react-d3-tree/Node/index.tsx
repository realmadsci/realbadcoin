...
import DefaultNodeElement from './DefaultNodeElement.tsx';
...
export default class Node extends React.Component<NodeProps, NodeState> {
  ...
  shouldComponentUpdate(nextProps: NodeProps, nextState: NodeState) {
    return nextState.wasClicked !== this.state.wasClicked || this.shouldNodeTransform(this.props, nextProps);
  }
  ...
  render() {
    ...
    return (
      <g
        ...
        className={[data.children ? 'rd3t-node' : 'rd3t-leaf-node', nodeClassName, data?.attributes?.cssClasses?.join(' ') ?? ""].join(' ').trim()}
